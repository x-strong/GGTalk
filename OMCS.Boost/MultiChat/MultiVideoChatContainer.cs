...
namespace OMCS.Boost.MultiChat
{
    ...
    {       
        ...
        private IMemberNameGetter memberNameGetter;
        public IChatGroup ChatGroup { get { return this.chatGroup; } }
        ...
        public void Initialize(IMultimediaManager mgr, string chatGroupID , IMemberNameGetter getter)
        {             
            this.multimediaManager = mgr;
            this.memberNameGetter = getter;
            ...
            myselfPanel.VideoDoubleClicked += new CbGeneric<SpeakerVideoPanel, IChatUnit>(panel_VideoDoubleClicked);
            this.flowLayoutPanel1.Controls.Add(myselfPanel);
            myselfPanel.Initialize(this.chatGroup.MyChatUnit, true, this.memberNameGetter.GetMemberName(this.chatGroup.MyChatUnit.MemberID));
            foreach (IChatUnit unit in this.chatGroup.GetOtherMembers())
            {
                SpeakerVideoPanel panel = new SpeakerVideoPanel();
                panel.VideoDoubleClicked += new CbGeneric<SpeakerVideoPanel, IChatUnit>(panel_VideoDoubleClicked);
                this.flowLayoutPanel1.Controls.Add(panel);
                panel.Initialize(unit, false, this.memberNameGetter.GetMemberName(unit.MemberID));
            }
        ...
        void panel_VideoDoubleClicked(SpeakerVideoPanel speakerVideoPanel, IChatUnit chatUnit)
        {
            //FullScreenVideoForm form = new FullScreenVideoForm(speakerVideoPanel, chatUnit);
            //form.ShowDialog();

            if (this.VideoDoubleClicked != null)
            {
                this.VideoDoubleClicked(speakerVideoPanel, chatUnit);
            }
        }              
    ...
    public interface IMemberNameGetter
    {
        string GetMemberName(string memberID);
    }

    public class DefaultMemberNameGetter : IMemberNameGetter
    {
