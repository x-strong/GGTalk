...
namespace ESFramework.Boost.NetworkDisk.Server
{
    ...
    public class NDiskHandler : IIntegratedCustomizeHandler
    {
        ...
        #region Initialize
        public void Initialize(IBaseFileController controller, NetworkDisk disk, IAgileLogger logger)
        {           
        ...
        {
            this.logger.LogWithTime(string.Format("进入HandleQuery方法 sourceUserID:{0},informationType:{1}", sourceUserID, informationType));
            try
            {
                #region ReqDirectory
                ...
                if (informationType == this.fileDirectoryInfoTypes.ReqNetworkDiskState)
                {
                    ...
                    this.logger.LogWithTime("处理ReqNetworkDiskState中 GetNetworkDiskState方法执行完成！");
                ...
                if (informationType == this.fileDirectoryInfoTypes.Download)
                {
                    ...
                    string fileOrDirPath = Comment4NDisk.GetRealPath(this.networkDisk.GetNetworkDiskRootPath(sourceUserID, contract.NetDiskID) + contract.SourceRemotePath);
                    string error = "";
                    try
                    {
            ...
            {
                this.logger.LogWithTime("HandleQuery方法执行出错 informationType=" + informationType + "原因：" + ee.Message);
            }
