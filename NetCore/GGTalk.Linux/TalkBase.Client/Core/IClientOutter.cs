...
namespace TalkBase.Client
{
    ...
    /// 与服务器/其它在线用户进行交互的通道。
    /// </summary>
    internal interface IClientOutter
    {
        #region Event
        ...
        /// <summary>
        /// 当收到对方处理我的好友申请时，触发该事件。 参数：destUserID - isRequester - isAgreed
        /// </summary>
        ...
        /// <summary>
        /// 当与某个Unit最后交谈记录发生变化时，触发此事件。 bool：是发送还是收到的消息  true：收到的新消息， false：自己发送的消息（在触发此事件之前，已经为目标Unit的LastWordsRecord属性赋值）
        ...
        /// 发送聊天消息。（并插入本地聊天记录）
        /// </summary>      
        ChatMessageRecord SendChatMessage(string destUserID, byte[] msg);

        /// <summary>
        /// 发送聊天消息（接收者可以为多个User和Group，用于转发消息）。（并插入本地聊天记录）
        /// </summary>      
        void SendChatMessage(List<string> accepters, byte[] msg);
        ...
        /// 发送群聊天消息。（并插入本地聊天记录）
        /// </summary>      
        ChatMessageRecord SendGroupChatMessage(string groupID, byte[] msg, string tag = null);
        ...
        /// </summary>        
        void ChangeMyHeadImage(int defaultHeadImageIndex, byte[] customizedHeadImage);
