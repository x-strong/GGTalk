...
namespace TalkBase.Server
{
        ...
        where TGroup : TalkBase.IGroup
    {
        ...
        /// 插入聊天记录成功
        /// </summary>
        public event CbGeneric<ChatBoxContent> StoreChatRecordSucceed;
        ...
        {
            int recordID = this.dbPersister.InsertChatMessageRecord(new ChatMessageRecord(senderID, accepterID, content, false));

            if (this.StoreChatRecordSucceed != null)
            {
                ChatBoxContent msg = ESPlus.Serialization.CompactPropertySerializer.Default.Deserialize<ChatBoxContent>(content, 0);
                msg.RecordID = recordID;
                this.StoreChatRecordSucceed(msg);
            }            
        }
        ...
        {
            int recordID = this.dbPersister.InsertChatMessageRecord(new ChatMessageRecord(senderID, groupID, content, true));
            if (this.StoreChatRecordSucceed != null)
            {
                ChatBoxContent msg = ESPlus.Serialization.CompactPropertySerializer.Default.Deserialize<ChatBoxContent>(content, 0);
                msg.RecordID = recordID;
