...
{   
    ...
    public class DBPersister : DefaultChatRecordPersister, IDBPersisterExtend
    {
        ...
        public List<GGUser> GetUserList4Phone(string phone)
        {
            ...
            {
                IOrmAccesser<GGUser> accesser = scope.NewOrmAccesser<GGUser>();
                userList = accesser.GetMuch(new Filter(GGUser._Phone, phone));
                scope.Commit();
            }
            return userList;
        }
        ...
        public int GetUserCount4Phone(string phone)
        {
            ...
            {
                IOrmAccesser<GGUser> accesser = scope.NewOrmAccesser<GGUser>();
                userCount = (int)accesser.GetCount(new Filter(GGUser._Phone, phone));
                scope.Commit();
            }
            return userCount;
        }
    ...
    public class DBPersister_SqlSugar : DefaultChatRecordPersister2, IDBPersisterExtend
    {
        ...
        private void ConnectionDb(DbType dbType)
        {
            ...
            {
                connectionString = string.Format("server={0},{1};uid={2};pwd={3};database={4}", ConfigurationManager.AppSettings["DBIP"], ConfigurationManager.AppSettings["DBPort"], ConfigurationManager.AppSettings["DBUserID"], ConfigurationManager.AppSettings["DBPwd"], ConfigurationManager.AppSettings["DBName"]);
            }
            //到时在加工
        ...
        public List<GGUser> GetUserList4Phone(string phone)
        {
            return db.Queryable<GGUser>().Where(it => it.Phone == phone).ToList();
        }
    ...
    public class DefaultChatRecordPersister2 : IChatRecordPersister
    {
        ...
        {
            ChatMessageRecord chatMessageRecord = db.Queryable<ChatMessageRecord>().Where(it => it.AutoID == id).First();
        ...
        public ChatRecordPage GetChatRecordPage(ChatRecordTimeScope chatRecordTimeScope, string myID, string friendID, int pageSize, int pageIndex)
        {
            ...
            return this.GetChatRecordPage(timeScope, myID, friendID, pageSize, pageIndex);
        }
