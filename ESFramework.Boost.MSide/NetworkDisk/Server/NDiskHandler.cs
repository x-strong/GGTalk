        ...
        public byte[] HandleQuery(string sourceUserID, ClientType clientType, int informationType, byte[] info)
        {
            ...
            try
            {
                ...
                if (informationType == this.fileDirectoryInfoTypes.Download)
                {
                    ...
                    this.fileController.BeginSendFile(sourceUserID, fileOrDirPath, Comment4NDisk.BuildComment(contract.SaveLocalPath, contract.NetDiskID), new ESPlus.FileTransceiver.SendingFileParas(10240,5), out projectID);
